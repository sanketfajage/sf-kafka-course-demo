...
public class ElasticSearchConsumer {
    ...
    private static String host = "kafka-testing-cluste-8048866808.ap-southeast-2.bonsaisearch.net";
    private static String accessKey = "5cj46odpjr";
    private static String accessSecret = "t18yg2hlfh";
    ...
    private static RestHighLevelClient createRestHighLevelClient() {
        ...
        RestClientBuilder restClientBuilder = RestClient.builder(new HttpHost(host, 443, "https")).setHttpClientConfigCallback(new RestClientBuilder.HttpClientConfigCallback() {
            @Override
    ...
    private static void insertTweetsToElasticSearch(RestHighLevelClient restHighLevelClient, ConsumerRecords<String, String> consumerRecords) {
        BulkRequest bulkRequest = new BulkRequest();
        for (ConsumerRecord<String, String> record : consumerRecords) {
            try {
                String tweetId = extractTweetId(record.value());
                IndexRequest indexRequest = new IndexRequest("tweeter", "tweets").source(record.value(), XContentType.JSON, tweetId);
